...
listCovNode = [(-7, 8),(8,8),(2, 8), (8, -9), (4, -5), (8, 15)]
    ...
    for nodeC in listCovNode:
        ...
        for edge in G.edges:
            ...
            if a2*b1-a1*b2 != 0:
                ...
                if (min(nodeC[0],nodeL[0]) <= x <= max(nodeL[0], nodeC[0]) and min(nodeC[1],nodeL[1]) <= y <= max(nodeL[1], nodeC[1])) and (min(edge[0][0],edge[1][0]) <= x <= max(edge[0][0],edge[1][0]) and  min(edge[0][1],edge[1][1]) <= y <= max(edge[0][1],edge[1][1])) and (nodeL[0] != x or nodeL[1] != y) :
                    check = False
                    print("The lidar",nodeL, "can not reach the point",nodeC, "because of the obstacle",edge)
                    
                    G2.add_node((x, y))
        if check == True:
            listEdge.append((nodeL, nodeC))
#def 
