...
#     # ((0,-10), (0,5), {'lidarRange': 0, 'priority': 3, 'width': 2}),
#     # ((-9,3), (-3,3), {'lidarRange': 0, 'priority': 2, 'width': 2}),

    ((-5,-10), (-5,-5),  {'lidarRange': 8, 'priority': 0, 'width': 0}),
    ((-5,-5), (-10,-5),  {'lidarRange': 8, 'priority': 0, 'width': 0}),
    ((-10,-5), (-10,5),  {'lidarRange': 8, 'priority': 0, 'width': 0}),
    ((-10,5), (5,5),  {'lidarRange': 8, 'priority': 0, 'width': 0}),
    ((5,5), (5,-10),  {'lidarRange': 8, 'priority': 0, 'width': 0}),
    ((5,-10), (-5,-10),  {'lidarRange': 8, 'priority': 0, 'width': 0})
    
    # ((-10, -10),(-10,10), {'lidarRange': 12, 'priority': 0, 'width': 0}),
    # ((-10,10), (10,10), {'lidarRange': 12, 'priority': 0, 'width': 0}),
    # ((10,10), (10,-10), {'lidarRange': 12, 'priority': 0, 'width': 0}),
    # ((10,-10), (-10,-10), {'lidarRange': 12, 'priority': 0, 'width': 0}),
    ])
# Gephi.add_node((5,5), width= 1, priority =0)
# Gephi.add_node((-5,-5), width= 1, priority =0)
# Gephi.add_node((5,-5), width= 1,priority =0)
# Gephi.add_node((-5,5), width= 1, priority =0)
# Gephi.add_node((0,0), width= 2, priority =2)
# Gephi.add_node((-8,-8), width= 2, priority =3)
Gephi.add_node((0,-3), width= 2, priority =0)
...
def MonitoringArea(Gephi: nx.Graph):
    ...
    while listNewNode != []:
        ...
        for element in listToVisit:
            for index in HMZ:
                #print(element[0], element[1], index[0], index[1], index[2], index[3])
...
def lidarJoint(listNodeStreet : List[Node], listLidar: List[Node]):
    ...
    #print(listCoveringEdge)
    return listCoveringEdge


# Graph,listLidar,listCovering, listEdge = StreetEdge(StreetPositionCovering(LidarPositioningPossibilities(graphBordersStreet(intersectionGenerator(neighborGenerator(Gephi))))))
...
with open('C:/Users/CYTech Student/workspace/graph.json', "w") as outfile:
    outfile.write(json_object)

# print("listCovering = ", listCovering)
# print("listCoveringEdge = ", listCoveringEdge)
Graph.add_nodes_from(listCovering)
Graph.add_edges_from(listCoveringEdge)
# pos5 = {node: node for node in Gephi.nodes}
# nx.draw(Gephi,  pos5, with_labels=False, node_color ='blue', node_size = 10)
# nx.draw_networkx_nodes(Gephi, pos5, liste, node_color='red',  node_size=40, label='Lidar', alpha= 0.5)

# nx.draw_networkx_nodes(Gephi, pos5, listCovering, node_color='purple',  node_size=40, label='Lidar', alpha= 0.5)
# plt.xlim(-20, 20)
# plt.ylim(-20, 20)
